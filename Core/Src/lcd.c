...
{	
	 ...
	 *
	 */
	//as is just locating one dot, so no need do it for the whole range.
	//create memory window
	LCD_Write_Cmd(0x2A);
	LCD_Write_Data(usCOLUMN >> 8 );//shift the 16 bit to right to get the top 8 bit for checking how many 8 bit is exceed
	LCD_Write_Data(usCOLUMN & 0xFF); // get the lower 8 bits for the dot
	LCD_Write_Data(usCOLUMN >> 8 );//shift the 16 bit to right to get the top 8 bit for checking how many 8 bit is exceed
	LCD_Write_Data(usCOLUMN & 0xFF);

	LCD_Write_Cmd(0x2B);
	LCD_Write_Data(usPAGE >> 8 );//shift the 16 bit to right to get the top 8 bit for checking how many 8 bit is exceed
	LCD_Write_Data(usPAGE & 0xFF); // get the lower 8 bits for the dot
	LCD_Write_Data(usPAGE >> 8 );//shift the 16 bit to right to get the top 8 bit for checking how many 8 bit is exceed
	LCD_Write_Data(usPAGE & 0xFF);
...
{	
	 ...
	 */
	//loop until back to the original point
	//calculate the first point first
	double error = 0.039; // as some not straightly equals to 1
	double ans;
	double num;
	for(uint16_t x = usC-LR; x <= usC+LR; x++)//loop through every pixel in the area
	{
		for(uint16_t y = usP-SR; y <= usP+SR; y++)
		{
			ans = (((x - usC) * (x - usC)) / (double)(LR * LR)) + ((y - usP) * (y - usP)) / (double)(SR * SR);//equation for ellipse
			//only (ans-1)<error draws a solid ellipse
			//0.9999 also accept? Also accept close negative after deduction
			//now only larger accept, smaller also need
			num = ans-1.0;
			if(num <0)//get absolute value after deduct to also included the near to one of both sides
			{
				num = -num;
			}
			if(num<error)//control how many decimal place we accept
			{
